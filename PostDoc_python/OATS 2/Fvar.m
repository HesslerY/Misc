...
%for u=1:length(RR)
    ...
    for o=1:MC
        ...
        for i=1:length(THETAac)
            ...
            Ethac(o,i,:)= Exx*cos(THETAac(i))*cos(PHIac(i))+Eyy*cos(THETAac(i))*sin(PHIac(i))-Ezz*sin(THETAac(i));
            Ephac(o,i,:)= -Exx*sin(PHIac(i))+Eyy*cos(PHIac(i));
            Erac(o,i,:) = Exx*sin(THETAac(i))*cos(PHIac(i))+Eyy*sin(THETAac(i))*sin(PHIac(i))+Ezz*cos(THETAac(i));
...
for i=[50,100]
    ...
    for j=1:1:MC
        A=[abs(Ethac(j,:,i));abs(Ephac(j,:,i))];
        B=A(:)/mean(A(:));
        [ff,x] = ecdf(B/mean(B)*sqrt(pi/2));
        stairs(x,ff)
        
    end
    xlim([0 4])
plot(X,th,'--r','LineWidth',2)
title(['$f=$ ',num2str(f(i)/1e6),' MHz, 50 simulations'],'Interpreter','Latex','Fontsize',12)
