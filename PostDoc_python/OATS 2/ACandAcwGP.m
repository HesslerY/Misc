...
for lll=1:.2:6
    ...
    for i=1:length(theta)
        for j=1:length(phi)
            ...
            Ethacgp(i,j,:)= Exx*cos(THETAac(i,j))*cos(PHIac(i,j))+Eyy*cos(THETAac(i,j))*sin(PHIac(i,j))-Ezz*sin(THETAac(i,j));
            Ephacgp(i,j,:)= -Exx*sin(PHIac(i,j))+Eyy*cos(PHIac(i,j));
            Eracgp(i,j,:) = Exx*sin(THETAac(i,j))*cos(PHIac(i,j))+Eyy*sin(THETAac(i,j))*sin(PHIac(i,j))+Ezz*cos(THETAac(i,j));
    ...
    for i=1:length(f)
        Fa2=abs(Ephacgp(:,:,i)).^2+abs(Ethacgp(:,:,i)).^2;
        Fa=Fa2/max(max(Fa2));
        omega = sum(sum((Fa) .* sin(THETAac)*dtheta*dphi));
        Dacgp(i,k,:) = 4*pi / omega;
    end
    toc
end

for i=1:26
plot(f,Dacgp(:,i)./Dac(:,i))
getframe;
end
%
% plot(f,Dac)%,f,Doats)
