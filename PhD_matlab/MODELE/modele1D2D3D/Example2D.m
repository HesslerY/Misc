...
filename = sprintf('%delem_%dns2D.mat',nbre_elements,round(Lt/(1e-9))); 
load(filename)
...
N = round(100*Lt*f0) %number of points for the chosen time-window (Lt)
...
subplot(2,1,2)
plot(freq/1e6,20*log10(FFTz/max(FFTz)))
title('FFT(CIR)')
xlim([0 500])
grid on
