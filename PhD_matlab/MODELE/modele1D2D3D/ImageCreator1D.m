%1D.m standalone script that generates the images for a l,p,h
%rectangular cavity
...
clear all
tic
Lt = 10e-6 %Time-window length in seconds
...
Ltp = Lt+3*l/c; %security margin in order to not truncate the sources too much
...
ordre = round(dmax/l)+1; %Maximum order
...
for z=1:1:length(X)
    for i=1:ordre
        POSp = [POSp; 	2*i*l-X(z)		    abs(2*i)-1		pi-tilt(z)  mod(2*pi-azimut(z),2*pi);
            			2*i*l+X(z)			abs(2*i)      	tilt(z)     mod(azimut(z),2*pi)];
    end
...
filename = sprintf('%delem_%d_1D.mat',length(X),round(Lt/(1e-9)));
save(filename,'POS')
toc
disp('Done.')
