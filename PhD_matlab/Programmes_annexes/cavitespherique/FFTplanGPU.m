...
Szgpu=GPUsingle(Sz);
        ...
        %         %---------FFT-----------------------------%
                ...
                CARTOfxH(dd,ee,:)=Ax(1:12000);
                CARTOfyH(dd,ee,:)=Ay(1:12000);
                CARTOfzH(dd,ee,:)=Az(1:12000);
        ...
        adf=ad/(length(X_1)*length(Y_1))*100;
        %
        ad=ad+1;
        %         RF=[f;A]';
        %
...
Y_1=-.15:.00025:.15;
Z_1=-.15:.00025:.15;
for dd=1:1:length(Y_1)
    for ee=1:1:length(Z_1)
        if (Y_1(dd)^2+Z_1(ee)^2)<=0.0225
        ...
        DY=Y_1(dd)-POS(j,2);%D(j,2);
        DZ=Z_1(ee)-POS(j,3);%D(j,3);
...
%         clear Signalz;
        ...
        %         %---------FFT-----------------------------%
                ...
                CARTOfxV(dd,ee,:)=Ax(1:12000);
                CARTOfyV(dd,ee,:)=Ay(1:12000);
                CARTOfzV(dd,ee,:)=Az(1:12000);
        ...
        adf=ad/(length(Y_1)*length(Z_1))*100;
        %
        ad=ad+1;
        %         RF=[f;A]';
        %
...
save('CARTOHHDezgwen200GPU.mat','f','CARTOfxH','CARTOfyH','CARTOfzH')
save('CARTOVHDezgwen200GPU.mat','f','CARTOfxV','CARTOfyV','CARTOfzV')

%plot(t,cumsum(Signalx.^2),t,cumsum(Signaly.^2),t,cumsum(Signalz.^2))
movieChampzHD
