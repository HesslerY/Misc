...
sig=std(k);
    ...
    Kfinal50100=[Kfinal50100;kmoy sig min95 max95];

end
for i=1:1:length(Kfinal50100(:))
    if Kfinal50100(i)<0
        Kfinal50100(i)=0
...
for f=1:1:length(v2);
    ...
    %Génération du S21
    Sr=sigma*(randn(N,Nexp))+(sqrt(v2(f)))*cos(45);
    Si=sigma*(randn(N,Nexp))+(sqrt(v2(f)))*sin(45);
    ...
    kmoy=mean(k,3)-1/N;
sig=std(k);
    min95=prctile(kmoy,5);
    max95=prctile(kmoy,95);

    Kfinal501=[Kfinal501;kmoy sig min95 max95];

end
for i=1:1:length(Kfinal501(:))
    if Kfinal501(i)<0
        Kfinal501(i)=0;
...
plot(K,10*log10(Kfinal50100(:,1)),'b')
plot(K,10*log10(Kfinal50100(:,3)),'b')
plot(Kvrai_dB,y(2,:),'b.');
plot(K,10*log10(Kfinal50100(:,3)),'b')
plot(K,10*log10(Kfinal501(:,4)),'g')
plot(K,10*log10(Kfinal501(:,3)),'g')
plot(Kvrai_dB,y(3,:),'b.');
