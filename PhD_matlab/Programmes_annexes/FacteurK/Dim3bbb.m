...
q025_lin=Kvrai_lin-1.96*(1-1/N)*sqrt(1/(N^2)+2*Kvrai_lin/N)/sqrt(Nfreq);
q975_lin=Kvrai_lin+1.96*(1-1/N)*sqrt(1/(N^2)+2*Kvrai_lin/N)/sqrt(Nfreq);
...
for f=1:1:length(v2);
    ...
    Kfinal2=[Kfinal2;kmoy min95 max95];
...
%******************
Kvrai_dB2=-40:1:30;
Kvrai_lin2=10.^(Kvrai_dB/10);
q025_lin2=Kvrai_lin-1.96*(1-1/N)*sqrt(1/(N^2)+2*Kvrai_lin/N)/sqrt(Nfreq);
q975_lin2=Kvrai_lin+1.96*(1-1/N)*sqrt(1/(N^2)+2*Kvrai_lin/N)/sqrt(Nfreq);
...
q025_dB2=10*log10(q025_lin2);
q975_dB2=10*log10(q975_lin2);
y2=[Kvrai_dB;q025_dB2;q975_dB2];
%******************
...
plot(K,10*log10(Kfinal(:,2)),'--b')
plot(Kvrai_dB,y(2,:),':b');
plot(K,10*log10(Kfinal2(:,1)),'g+')
plot(K,10*log10(Kfinal2(:,2)),'g')
plot(Kvrai_dB,y2(2,:),'-.g');
plot(K,10*log10(Kfinal(:,3)),'--b')
plot(Kvrai_dB,y(3,:),':b');
plot(K,10*log10(Kfinal2(:,3)),'g')
plot(Kvrai_dB,y2(3,:),'-.g')


legend('$K$','$\widehat{K}$ with $N=30$ and $N_\textrm{freq}=30$','95\% CI of $\widehat{K}$ with $N=30$ and $N_\textrm{freq}=30$','95\% theroretical CI of $\widehat{K}$ with $N=30$ and $N_\textrm{freq}=30$','$\widehat{K}$ with $N=50$ and $N_\textrm{freq}=100$','95\% CI of $\widehat{K}$ with $N=50$ and $N_\textrm{freq}=100$','95\% Theoretical CI of $\widehat{K}$ with $N=50$ and $N_\textrm{freq}=100$')
