...
for v=[6,3]
    ...
    for w=1:360
        disp([num2str(w),'/360'])
        tic
        filename = sprintf('y_%d_%d.mat',v,w);
        load(filename)
        for u=1:1:length(X)
            ...
            yFFTx(u,:)=abs(Yx(1:7000));
            yFFTy(u,:)=abs(Yy(1:7000));
            yFFTz(u,:)=abs(Yz(1:7000));
            freq=f(1:7000);

        end
        filename2=sprintf('Resulty_%d_%d.mat',v,w);
        save(filename2,'X','Y','Z','yFFTx','yFFTy','yFFTz','freq','f0')
        toc
        clear yFFTx yFFTy yFFTz
    end
end
