...
for f0=100e6:5e6:2000e6
frequence=round(f0/freq(2))+1;
    ...
    o2=optim(100,6);
    th2=optim(100,7);
    ph2=optim(100,8);%2*pi*rand;
    
    o3=optim(100,9);
    th3=optim(100,10);
    ph3=optim(100,11);%2*pi*rand;
    
    o4=optim(100,12);
    th4=optim(100,13);
    ph4=optim(100,14);%pi*rand;
    
    o5=optim(100,15);
    th5=optim(100,16);
    ph5=optim(100,17);%2*pi*rand;
    
    o6=optim(100,18);
    th6=optim(100,19);
    ph6=optim(100,20);%2*pi*rand;
    ...
    for u=1:length(fftx(1,:))
        A=corrcoef((fftx(:,length(fftx(1,:))/2)),(fftx(:,u)));
        B=corrcoef((ffty(:,length(ffty(1,:))/2)),(ffty(:,u)));
        C=corrcoef((fftz(:,length(fftz(1,:))/2)),(fftz(:,u)));
        Rx(u,sss)=A(2,1);
        Ry(u,sss)=B(2,1);
        Rz(u,sss)=C(2,1);
    end
   ...
   % xlim([0 360])
   % ylim([-.2 1.1])
    filename2=sprintf('ttrnd6palesgraphoptimRxRyRz30deg.mat')
    save(filename2,'Rx','Ry','Rz','phi')
    %filename=sprintf('6palesgraphoptimttrnd10deg_%d.png',sss)
    %saveas(gcf,filename,'png')
    optimisation=[optimisation; freq(frequence)/1e6  sum(abs(Rx(:,sss)))/360*q sum(abs(Ry(:,sss)))/360*q sum(abs(Rz(:,sss)))/360*q]
    save('optimisationttrnd30deg.mat','optim')
end
