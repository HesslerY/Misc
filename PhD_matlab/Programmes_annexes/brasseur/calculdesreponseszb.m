...
for v=[6,3]
    ...
    for w=1:360
        disp([num2str(w),'/360'])
        tic
        filename = sprintf('z_%d_%d.mat',v,w);
        load(filename)
        for u=1:1:length(X)
            ...
            zFFTx(u,:)=abs(Yx(1:7000));
            zFFTy(u,:)=abs(Yy(1:7000));
            zFFTz(u,:)=abs(Yz(1:7000));
            freq=f(1:7000);

        end
        filename2=sprintf('Resultz_%d_%d.mat',v,w);
        save(filename2,'X','Y','Z','zFFTx','zFFTy','zFFTz','freq','f0')
        toc
        clear zFFTx zFFTy zFFTz
    end
end
