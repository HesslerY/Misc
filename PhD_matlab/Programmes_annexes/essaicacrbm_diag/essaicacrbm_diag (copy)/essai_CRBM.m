...
%load antennerandom.mat

   load XYZ150b.mat

%    disp([num2str(ang),'/50'])
    alp=pi*rand(1,5000);
    bet=2*pi*rand(1,5000);
    
for uuu=1:50  
    [Sx,Sy,Sz,t,azim,elev] = CIRvectca(X(uuu),Y(uuu),Z(uuu));
...
fr = fres(mod(pi/2-elev,pi), mod(pi-azim,2*pi), n, xa, ya, za, Ia);
fa = fantres(fr,mod(pi/2-elev,pi),mod(pi-azim,2*pi), S);
