...
Z=[Z_z];
        ...
        for i=1:59
            X=[X;X_x+i*0.005/sqrt(3)];
            Y=[Y;Y_y+i*0.005/sqrt(3)];
            Z=[Z;Z_z+i*0.005/sqrt(3)];
        end
...
while (length(X)<6000)
    ...
    if min(D)>lambda/2
        ...
        Z=[Z;Z_0];
        for i=1:60
            X=[X;X_0+i*0.005/sqrt(3)];
            Y=[Y;Y_0+i*0.005/sqrt(3)];
            Z=[Z;Z_0+i*0.005/sqrt(3)];
        end
    end
end
...
disp([num2str(u),'/',num2str(length(X))])
   ...
   s111FFTx(u,:)=abs(Yx(1:10000));
   s111FFTy(u,:)=abs(Yy(1:10000));
   s111FFTz(u,:)=abs(Yz(1:10000));
   freq=f(1:10000);
   toc
end

save('ResultHill500elements_111.mat','X','Y','Z','s111FFTx','s111FFTy','s111FFTz','freq','f0')
toc
