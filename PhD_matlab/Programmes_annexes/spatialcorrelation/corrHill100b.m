...
while (length(X)<400)
   ...
   if min(D)>lambda/2
       ...
       X=[X;X_0+0.015/sqrt(3)];
       Y=[Y;Y_0+0.015/sqrt(3)];
       Z=[Z;Z_0+0.015/sqrt(3)];
   end
end
...
disp([num2str(u),'/',num2str(length(X))])
   ...
   FFTx(u,:)=abs(Yx(1:80000));
   FFTy(u,:)=abs(Yy(1:80000));
   FFTz(u,:)=abs(Yz(1:80000));
   freq=f(1:80000);
   toc
end

save('ResultHill4x100elementsb.mat','X','Y','Z','FFTx','FFTy','FFTz','freq','f0')
toc