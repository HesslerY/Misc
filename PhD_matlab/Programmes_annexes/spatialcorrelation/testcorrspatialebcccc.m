...
s111FFTz(1:60,:)=[];
k=0;
for f=526:578%1050:1102%f=1000:1:1050
    k=k+1
    for p=1:61
...
Resultabs(k,:)=Result;    
    Resultll(k,:)=mean(Resultl);
    Resulttt(k,:)=mean(Resultt);
    Resultnn(k,:)=mean(Resultn);
    %    p=1:61;
...
d=mean(Resultabs);
rttth=((3/2*(sin(d)./d-1./d.^2.*(sin(d)./d-cos(d))))).^2;
rllth=(3./d.^2.*(sin(d)./d-cos(d))).^2;
rnnth=(sin(d)./d).^2;


% plot(mean(Resultabs),mean(Resultll),mean(Resultabs),rllth,mean(Resultabs),mean(Resulttt),mean(Resultabs),rttth,mean(Resultabs),mean(Resultnn),mean(Resultabs),rnnth)
%  %legend('$\rho_{ll}$','$\rho_{ll}$ th.','$\rho_{tt}$','$\rho_{tt}$ th.','$\rho^2$','$\rho^2$ th.')
%  
%    xlim([0 7.8])
%     ylim([-0.05 1.05])
...
%     ylabel('Corr.','Interpreter','Latex','FontSize',12)
  ...
  figure(2)
 plot(mean(Resultabs),Resultll(:,:)','-b',mean(Resultabs),rllth,'-r')
 xlim([0 pi])
    ylim([-0.1 1.05])
   % h=legend('$\rho_{ll}$ sim.','$\rho_{ll}$ th.','$\rho_{tt}$ sim.','$\rho_{tt}$ th.','$\rho^2$ sim.','$\rho^2$ th.')
   % set(h,'Interpreter','Latex')
